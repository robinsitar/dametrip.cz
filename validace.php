    ...
    <?php
        ...
        if($ok) {
            ...
            if($partak && $partak[9]<=$range) {
                ...
                posliMail($ja[4],"Dámetrip.cz - Našli jsme vám parťáka!","Zdravím dobrodruhu <strong>" . $ja[1] . "</strong>,<br />Po důkladném prohledání naší databáze jsme ti našli dokonalého parťáka. Tady jsou základní informace, které se můžou hodit! <br><br> Jmenuje se <strong>" . $partak[1] . "</strong>. Jako doma se cítí v <strong>" . geo2human($partak[3]) . "</strong>. Je mu přesně <strong>" . $partak[5] . "</strong> let. Rád by chtěl jet do <strong>" . geo2human($partak[2]) . "</strong> a to hlavně kvůli <strong>" . partak[6] . "</strong>. <br />Rozhodně mu nezapomeňte napsat na jeho email: <strong>" . $partak[4] . "</strong>! <br> Vyražte spolu na super trip a užíte si co nejvíce nezapomenutelných zážitků!<br><br>Děkujeme, že jste využili Dámetrip.cz a přejeme mnoho krásných dobrodružstvích !");
                
                posliMail($partak[4],"Dámetrip.cz - Našli jsme vám parťáka!","Zdravím dobrodruhu <strong>" . $partak[1] . "</strong>,<br />Po důkladném prohledání naší databáze jsme ti našli dokonalého parťáka. Tady jsou základní informace, které se můžou hodit! <br><br> Jmenuje se <strong>" . $ja[1] . "</strong>. Jako doma se cítí v <strong>" . geo2human($ja[3]) . "</strong>. Je mu přesně <strong>" . $ja[5] . "</strong> let. Rád by chtěl jet do <strong>" . geo2human($ja[2]) . "</strong> a to hlavně kvůli <strong>" . ja[6] . "</strong>. <br />Rozhodně mu nezapomeňte napsat na jeho email: <strong>" . $ja[4] . "</strong>! <br> Vyražte spolu na super trip a užíte si co nejvíce nezapomenutelných zážitků!<br><br>Děkujeme, že jste využili Dámetrip.cz a přejeme mnoho krásných dobrodružstvích !");
                
                //infomail adminovi - až bude větší počet uživatelů, tak zrušit... zatím je to super na tweakování matchovací funkce
                posliMail($adminMail,"Dámetrip.cz - Parťáci spojeni","Spojila jsem další pár!\nPrvní týpek se jmenuje ".$ja[1].", je z ".geo2human($ja[3])." a jede do ".geo2human($ja[2]).". Je mu ".$ja[5]." a chce tam ".$ja[6]."\nDruhý týpek se jmenuje ".$partak[1].", je z ".geo2human($partak[3])." a jede do ".geo2human($partak[2]).". Je mu ".$partak[5]." a chce tam ".$partak[6]."\nAdios!","sylvia@dametrip.cz");
