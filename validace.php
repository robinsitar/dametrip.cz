<html>
    ...
    <?php
        ...
        if($ok)
        {
            ...
            $ja=mysqli_fetch_array(dotaz("SELECT Id, Jmeno, Destinace, Bydliste, Email, Vek FROM lidi WHERE Kod=$kod"))[0];
            $partak=matchni($ja[0]);
            if($partak[9]<=$range){
                ...
                posliMail($ja[4],"Dámetrip.cz - Registrace dokončena!","Ahoj ".$ja[1].",\nVítej v Dámetripu! Co nejdřívě ti najdeme nějakého parťáka... Pak se ti ozveme na tento email.\n Kluci z Dámetripu");
            }
